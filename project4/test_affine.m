...
while go
	...
	% Run each algorithm
	for l=1:length(alg_list)
		string = ['tic; fitting = ', alg_list{l}, '(noisy_img, tmplt_img, p_init, n_iters, verbose, opt_step_size); t = toc;'];
		eval(string);
		fitting(1);
		% Evaluate point spatial error for each iteration for convergence tests
		if cnvg_testing
			rms_pt_err = zeros(length(fitting), 1);
			for f=1:length(fitting)
				rms_pt_error(f) = ComputePointError(test_pts, template_affine, fitting(f).warp_p);
			end
					
		% Only need final spatial rms for divergence tests. Don't need fitting results
		else
			rms_pt_error = ComputePointError(test_pts, template_affine, fitting(end).warp_p);
			fitting = [];
		end	
...
% 			
		...
		results = setfield(results, {1}, alg_list{l}, {offset_idx}, 'fit', fitting);
